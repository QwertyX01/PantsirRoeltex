...
local function getNameColor(drone)
    local type = getDroneType(drone)
    if STATE.nameColorMode == "team" then
        return type == "Player" and Color3.fromRGB(0, 255, 100) or Color3.fromRGB(255, 60, 60)
...
local function startSpin()
    ...
    spinConnection = RunService.Heartbeat:Connect(function()
        ...
        if bav then bav.AngularVelocity = Vector3.new(0, STATE.spinSpeed, 0) end
    end)
end
...
local function startKillAll()
    ...
    while STATE.killAll do
        ...
        if #drones == 0 then task.wait(0.5); continue end
...
local function createDefTracer(plr)
    if defTracer[plr] then return end    local line = Drawing.new("Line")
    line.Thickness = 1.5
...
RunService.RenderStepped:Connect(function()
    ...
    for plr, _ in pairs(DEFENDERS) do
        ...
        if box then
            ...
            if vert then
                local letters = {}
                for c in string.gmatch("DEFENDERS", ".") do
                    table.insert(letters, c)
                end
                vert.Text = table.concat(letters, "\n")
...
-- 🟣 СТАВИМ ФИОЛЕТОВУЮ ТЕМУ (правильный синтаксис)
task.spawn(function()
    task.wait(0.1)
    pcall(function()
        ThemeManager:SetLibrary(Library)
